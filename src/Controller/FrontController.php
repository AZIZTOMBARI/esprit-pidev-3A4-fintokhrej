...
class FrontController extends AbstractController
{
    ...
    public function lieux(Request $request, Connection $connection): Response
    {
        $user = $this->getUser();
        $userId = $user instanceof User ? (int) $user->getId() : 0;

        $category = trim((string) $request->query->get('categorie', ''));
        $city = trim((string) $request->query->get('ville', ''));
        $type = trim((string) $request->query->get('type', ''));
        $budget = trim((string) $request->query->get('budget', ''));
        ...
        if ($budget !== '') {
            if ($budget === '0-20') {
                $where[] = 'COALESCE(l.budget_min, 0) <= 20';
            } elseif ($budget === '20-60') {
                $where[] = 'COALESCE(l.budget_max, 0) >= 20 AND COALESCE(l.budget_min, 0) <= 60';
            } elseif ($budget === '60-120') {
                $where[] = 'COALESCE(l.budget_max, 0) >= 60 AND COALESCE(l.budget_min, 0) <= 120';
            } elseif ($budget === '120+') {
                $where[] = 'COALESCE(l.budget_max, 0) >= 120';
            }
        }
        ...
        $places = $this->fetchAll($connection, "
            SELECT
                ...
                l.longitude,
                COALESCE(
                    (SELECT li.image_url FROM lieu_image li WHERE li.lieu_id = l.id ORDER BY li.ordre ASC, li.id ASC LIMIT 1),
                    l.image_url
                ) AS image_url,
                CASE
                    WHEN ? > 0 AND EXISTS(
                        SELECT 1 FROM favori_lieu fl WHERE fl.user_id = ? AND fl.lieu_id = l.id
                    ) THEN 1
                    ELSE 0
                END AS is_favorite
            FROM lieu l
        ...
        return $this->render('front/lieu/index.html.twig', [
            ...
            'categories' => $connection->fetchFirstColumn('SELECT DISTINCT categorie FROM lieu WHERE categorie IS NOT NULL AND categorie <> "" ORDER BY categorie ASC'),
            'cities' => $connection->fetchFirstColumn('SELECT DISTINCT ville FROM lieu WHERE ville IS NOT NULL AND ville <> "" ORDER BY ville ASC'),
            'types' => $connection->fetchFirstColumn('SELECT DISTINCT type FROM lieu WHERE type IS NOT NULL AND type <> "" ORDER BY type ASC'),
            'notificationData' => $this->getNotificationData($connection),
        ]);
    }
    ...
    #[Route('/lieux/{id}/favori', name: 'app_lieu_favorite_toggle', methods: ['POST'], requirements: ['id' => '\\d+'])]
    public function toggleLieuFavorite(int $id, Request $request, Connection $connection): Response
    {
        $user = $this->getUser();
        if (!$user instanceof User) {
        ...
        $exists = (int) $connection->fetchOne('SELECT COUNT(*) FROM lieu WHERE id = ?', [$id]);
        if ($exists === 0) {
            throw $this->createNotFoundException('Lieu introuvable.');
        }

        try {
            $favoriteId = $connection->fetchOne(
                'SELECT id FROM favori_lieu WHERE user_id = ? AND lieu_id = ? LIMIT 1',
                [$user->getId(), $id]
            );

            if ($favoriteId !== false && $favoriteId !== null) {
                $connection->delete('favori_lieu', ['id' => (int) $favoriteId]);
                $this->addFlash('success', 'Lieu retiré de vos favoris.');
            } else {
                $connection->insert('favori_lieu', [
                    'user_id' => $user->getId(),
                    'lieu_id' => $id,
                    'date_ajout' => (new \DateTimeImmutable())->format('Y-m-d H:i:s'),
                ]);
                $this->addFlash('success', 'Lieu ajouté à vos favoris.');
            }
        } catch (\Throwable) {
            $this->addFlash('error', 'Impossible de mettre à jour vos favoris pour le moment.');
        }

        $redirectUrl = trim((string) $request->request->get('_redirect', ''));
        if ($redirectUrl !== '' && str_starts_with($redirectUrl, '/lieux')) {
    ...
    #[Route('/api/reviews/moderation-preview', name: 'app_review_moderation_preview', methods: ['POST'])]
    public function moderationPreview(Request $request, ReviewContentModerator $contentModerator): JsonResponse
    {
        ...
        $analysis = $contentModerator->analyze($text);

        return $this->json([
            'ok' => true,
            'hasViolation' => (bool) $analysis['hasViolation'],
    ...
    private function denyToxicReview(Connection $connection, array $analysis, string $rawComment, int $lieuId, int $userId, LoggerInterface $logger, string $action): bool
    {
        ...
        $terms = array_map(static fn (array $m): string => (string) $m['term'], $analysis['matches']);
        $terms = array_values(array_unique($terms));

        $severity = (string) ($analysis['severity'] ?? 'moderate');
        $severityLabel = $severity === 'severe' ? 'sévère' : 'modéré';
        $score = (int) ($analysis['score'] ?? 0);
        ...
        $logger->warning('review_toxicity_blocked', [
            ...
            'comment_preview' => mb_substr($rawComment, 0, 160),
     ...
     * @param string[] $terms
     */
    private function storeModerationAttempt(
        ...
        try {
            ...
            $connection->insert('moderation_attempt_log', [
                'user_id' => $userId,
                'lieu_id' => $lieuId,
                'action' => $action,
                'severity' => $severity,
                'score' => $score,
                'terms_json' => json_encode(array_values(array_unique($terms)), JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
