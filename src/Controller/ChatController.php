...
class ChatController extends AbstractController
{
    ...
    public function createPoll(int $id, Request $request, Connection $connection): RedirectResponse
    {
        ...
        $connection->insert('chat_message', [
            ...
            'sender_id' => (int) $user->getId(),
            'content' => 'a cree un nouveau sondage.',
            'message_type' => 'POLL',
            'poll_id' => $pollId,
            'meta_json' => null,
    ...
    public function createTask(int $id, Request $request, Connection $connection): RedirectResponse
    {
        ...
        if ($assignedTo > 0 && !$this->isChatMember($connection, $id, $assignedTo)) {
            $assignedTo = 0;
        }
    ...
    public function toggleTask(int $id, int $taskId, Request $request, Connection $connection): RedirectResponse
    {
        ...
        $task = $connection->fetchAssociative(
            'SELECT id, status FROM sortie_task WHERE id = ? AND annonce_id = ? LIMIT 1',
            [$taskId, $id]
        );
    ...
    private function fetchMembers(Connection $connection, int $sortieId): array
    {
        return $connection->fetchAllAssociative(
            'SELECT DISTINCT u.id, u.prenom, u.nom, u.imageUrl AS image_url,
                    CASE WHEN s.user_id = u.id THEN 1 ELSE 0 END AS is_creator
             FROM chat_groupe cg
             INNER JOIN chat_groupe_membre cgm ON cgm.chat_groupe_id = cg.id
             INNER JOIN user u ON u.id = cgm.user_id
             INNER JOIN annonce_sortie s ON s.id = cg.annonce_id
             WHERE cg.annonce_id = ?
             ORDER BY is_creator DESC, u.prenom ASC, u.nom ASC',
            [$sortieId]
        );
    }
    ...
    {
        return $connection->fetchAllAssociative(
            'SELECT st.id, st.title, st.description, st.status, st.created_at, st.updated_at, st.done_at,
